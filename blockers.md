...
purpose — decisions #1, #2, #3, #5, #6, #7, #8, #9, #10, #12 and #13 are fully settled and have
moved to §4; #4 and #11 keep their original numbers rather than being renumbered, so every
cross-reference elsewhere in this file (and in commit messages already pushed) still points at the
right row.
...
|---|---|---|---|---|---|---|---|---|
| 11 | **Role → critical-areas table for `critical_areas()` — DESIGN.md §6.1 gives one example sentence, not a full table** | §6.1 says "role drives question priority" and gives exactly one example: an engineer answers `nfr`/`data`/`platform` directly but needs guidance on `success`; product inverts. No full table exists in DESIGN.md. Filled by assumption so T04 could ship a working `critical_areas()`: `problem`/`users` always critical for every role (§6.4: "must ask, never assume"); engineer adds `nfr`/`data`/`platform`; product adds `success`/`scope_in`/`scope_out`; `mixed` takes the union of both. `technical_depth`/`domain_familiarity` are accepted on `UserProfile` but don't currently narrow the set further. | 2026-09-23 | 17:10 IST | **Taken (needs confirmation)** — same posture as decision #7: an assumption filling a real spec gap, not silently buried | Not testable against DESIGN.md directly since DESIGN.md gives no full table — tests in `tests/test_config/test_profiles.py` assert only what the one example sentence requires (engineer covers `nfr`/`data`/`platform`; product includes `success`; both include `problem`/`users`; the two sets differ). A rename or re-grouping is a one-line change to `_ROLE_CRITICAL_EXTRA` in `ppa/config/profiles.py`, not a redesign. | **Developer: please confirm or override this table**, particularly whether `mixed` should be a union (current choice) or its own distinct set, and whether `technical_depth`/`domain_familiarity` should narrow it further. Nothing downstream depends on the exact areas yet — T10 (readiness engine) will start reading `critical_areas()` output soon. | `t04_config_areas_profiles_house_style.md` |
...
eval runs — pending, resolves at T34) and decision #11 (the role → critical-areas table from T04 —
taken, but flagged as needing your confirmation, not a closed matter). Both stay in §2 exactly
because they're not done; moving them here would misrepresent that.
